-- =========================================================
-- Reino de Eldoria - Patch de diagnostico e correcoes
-- Data: 2026-03-09
-- =========================================================
...
-- ---------------------------------------------------------
-- 1) Game save: moeda premium persistida
-- ---------------------------------------------------------
alter table public.game_saves
  add column if not exists crown_diamonds numeric not null default 0 check (crown_diamonds >= 0);

update public.game_saves
set crown_diamonds = floor(coalesce(rebirth_currency, 0) / 1000)
where coalesce(crown_diamonds, 0) = 0
  and coalesce(rebirth_currency, 0) >= 1000;

create index if not exists idx_game_saves_crown_diamonds
  on public.game_saves (crown_diamonds desc);

-- ---------------------------------------------------------
-- 2) Username obrigatorio e valido
-- ---------------------------------------------------------
update public.profiles
set username = lower('aventureiro_' || substring(id::text from 1 for 8))
...
with prepared as (
  select
    p.id,
    case
      when length(lower(regexp_replace(coalesce(p.username, ''), '[^a-zA-Z0-9_]+', '', 'g'))) >= 3
        then left(lower(regexp_replace(coalesce(p.username, ''), '[^a-zA-Z0-9_]+', '', 'g')), 20)
      else lower('aventureiro_' || substring(p.id::text from 1 for 8))
...
ranked as (
  select
    pr.id,
    pr.base_username,
    row_number() over (partition by pr.base_username order by pr.id) as rn
  from prepared pr
),
resolved as (
  select
    ...
    case
      ...
      else left(r.base_username, 15) || '_' || lpad((r.rn - 1)::text, 4, '0')
    end as final_username
  from ranked r
)
update public.profiles p
set username = rs.final_username
from resolved rs
where p.id = rs.id;
...
begin
  ...
  if length(v_candidate) < 3 then
    v_candidate := lower('aventureiro_' || substring(new.id::text from 1 for 8));
  end if;

  v_candidate := left(v_candidate, 20);

  if exists (select 1 from public.profiles p where lower(p.username) = v_candidate) then
    v_suffix := '_' || substring(new.id::text from 1 for 4);
    v_candidate := left(v_candidate, 20 - length(v_suffix)) || v_suffix;
  end if;
...
-- ---------------------------------------------------------
-- 3) Rotacao de caixas e compra unica por rotacao
-- ---------------------------------------------------------
...
set
  ...
  price = case box_key
    ...
    when 'caixa_real' then 20
    when 'caixa_lendaria' then 65
    when 'caixa_celestial' then 140
    else price
  end,
  spawn_weight = case box_key
    when 'caixa_comum' then 70
    when 'caixa_mercador' then 45
    when 'caixa_guilda' then 24
    when 'caixa_real' then 10
    when 'caixa_lendaria' then 4
    when 'caixa_celestial' then 2
    else spawn_weight
  end,
  duration_minutes = case box_key
    when 'caixa_comum' then 140
    when 'caixa_mercador' then 120
    when 'caixa_guilda' then 105
    when 'caixa_real' then 85
    when 'caixa_lendaria' then 55
    when 'caixa_celestial' then 40
...
set
  min_quantity = 1,
  max_quantity = 1
where min_quantity <> 1 or max_quantity <> 1;
...
begin
  ...
  if v_state.next_spawn_at <= v_now then
    v_spawn_count := case when random() < 0.30 then 2 else 1 end;

    if exists (select 1 from public.loot_boxes lb where lb.active = true) then
      with picked as (
...
-- ---------------------------------------------------------
-- 4) Conversao de moeda no backend
-- ---------------------------------------------------------
create or replace function public.convert_seals_to_diamonds(p_diamonds integer)
returns jsonb
language plpgsql
security definer
set search_path = public
...
declare
  v_uid uuid := auth.uid();
  v_requested integer := greatest(coalesce(p_diamonds, 0), 0);
  v_required_seals numeric;
...
begin
  ...
  if v_requested <= 0 then
    raise exception 'Quantidade invalida para conversao.';
  end if;

  v_required_seals := v_requested * 1000;
  ...
  if coalesce(v_save.rebirth_currency, 0) < v_required_seals then
    raise exception 'Selos da Aurora insuficientes.';
  end if;

  update public.game_saves gs
  set
    rebirth_currency = gs.rebirth_currency - v_required_seals,
    crown_diamonds = gs.crown_diamonds + v_requested,
    last_save_at = timezone('utc', now())
  where gs.user_id = v_uid;

  return jsonb_build_object(
    'convertedDiamonds', v_requested,
    'spentSeals', v_required_seals,
    'rebirthCurrency', (select rebirth_currency from public.game_saves where user_id = v_uid),
    'crownDiamonds', (select crown_diamonds from public.game_saves where user_id = v_uid)
  );
end;
$$;

grant execute on function public.convert_seals_to_diamonds(integer) to authenticated;

-- ---------------------------------------------------------
-- 5) Abertura de caixa segura (compativel com assinatura antiga e nova)
-- ---------------------------------------------------------
...
begin
  ...
  v_price_diamonds := greatest(1::numeric, floor(coalesce(v_box.price, 1)));

  if coalesce(v_save.crown_diamonds, 0) < v_price_diamonds then
    raise exception 'Diamantes da Coroa insuficientes para abrir esta caixa.';
  end if;
  ...
  update public.game_saves gs
  set
    crown_diamonds = gs.crown_diamonds - v_price_diamonds,
    stats = jsonb_set(
  ...
  return jsonb_build_object(
    ...
    'pricePaidSeals', v_price_diamonds * 1000,
    'remainingRebirthCurrency', (select gs.rebirth_currency from public.game_saves gs where gs.user_id = v_uid),
    'remainingDiamonds', (select gs.crown_diamonds from public.game_saves gs where gs.user_id = v_uid)
  );
end;
$$;
...
declare
  ...
  v_rotation_id uuid;
begin
  if p_box_key is null or trim(p_box_key) = '' then
    raise exception 'Caixa invalida.';
  end if;
  ...
  return public.open_active_loot_box(v_rotation_id);
end;
$$;
...
-- ---------------------------------------------------------
-- 6) Equip por categoria e equipar melhores
-- ---------------------------------------------------------
...
create or replace function public.equip_best_items()
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  for v_choice in
    with ranked as (
      select
        ...
        idf.category as slot_category,
        row_number() over (
          partition by idf.category
          order by
            case
              when idf.passive_type in ('click_flat', 'passive_flat') then idf.passive_value
              else idf.passive_value * 100
            end desc,
            public.rarity_tier(idf.rarity) desc,
            idf.base_value desc,
            ui.acquired_at asc
        ) as rn
      from public.user_items ui
      ...
      where ui.user_id = v_uid
        and ui.is_locked_in_trade = false
        and (idf.stackable = false or ui.quantity = 1)
    )
    select
      r.user_item_id,
      r.slot_category
  ...
  loop
    ...
    set
      is_equipped = true,
      equipped_slot = v_choice.slot_category,
      is_marked_tradable = false,
      updated_at = timezone('utc', now())
    where id = v_choice.user_item_id;
...
grant execute on function public.equip_best_items() to authenticated;
