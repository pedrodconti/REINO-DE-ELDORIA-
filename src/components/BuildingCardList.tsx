...
import { getBuildingPixelArt } from '@/data/pixelArt';
            ...
            return (
              ...
              >
                <div className="mb-2 flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    <div className="rounded-md border border-border/70 bg-background/80 p-1.5">
                      <PixelArtSprite src={getBuildingPixelArt(building.id)} alt={building.name} size={28} />
                    </div>
                    <div>
                ...
                <div className="mb-3 flex flex-wrap items-center gap-1">
                  {Array.from({ length: Math.min(owned, 8) }).map((_, markerIndex) => (
                    <div
                      key={`${building.id}-marker-${markerIndex}`}
                      className="rounded border border-border/70 bg-background/70 p-0.5"
                    >
                      <PixelArtSprite src={getBuildingPixelArt(building.id)} alt={building.name} size={12} />
                    </div>
                  ))}
                  {owned > 8 ? (
                    <span className="text-[10px] text-muted-foreground">+{(owned - 8).toLocaleString('pt-BR')}</span>
                  ) : null}
                </div>
